...
import { fetchContent, hydrateContent } from './lib/content.ts';
...
function setupFluid(_shared: SharedState): FluidRenderer | null {
  ...
  const textureType: THREE.TextureDataType =
  ...
  const mAdvect = new THREE.ShaderMaterial({
    uniforms: {
      velocity: { value: null }, source: { value: null },
      px: { value: px }, dt: { value: 1 / 60 },
      scale: { value: 1 }, time: { value: 0 },
    },
  ...
  const mJacobi = new THREE.ShaderMaterial({
    uniforms: {
      pressure: { value: null }, divergence: { value: null },
      alpha: { value: -1 }, beta: { value: 0.25 }, px: { value: px },
    },
    vertexShader: VS, fragmentShader: JACOBI,
  });
  const mSubP = new THREE.ShaderMaterial({
    uniforms: {
      pressure: { value: null }, velocity: { value: null },
      px: { value: px }, scale: { value: 1 },
    },
  ...
  const mAdvDye = new THREE.ShaderMaterial({
    uniforms: {
      velocity: { value: null }, source: { value: null },
      px: { value: px }, dt: { value: 1 / 60 },
      dissipation: { value: FLUID_CFG.dyeDissipation },
    },
  ...
  const swapV = (): void => { [vFBO0, vFBO1] = [vFBO1, vFBO0]; };
  const swapP = (): void => { [pFBO0, pFBO1] = [pFBO1, pFBO0]; };
  const swapD = (): void => { [dFBO0, dFBO1] = [dFBO1, dFBO0]; };

  function splatDye(cx: number, cy: number, cr: RGB, intensity: number): void {
    ...
    mSplat.uniforms.scale.value.set(
      FLUID_CFG.cursorSize * px.x * 0.7,
  ...
  function addForce(cx: number, cy: number, fx: number, fy: number, radiusMult = 1): void {
    ...
    mForce.uniforms.scale.value.set(
      FLUID_CFG.cursorSize * radiusMult * px.x,
      FLUID_CFG.cursorSize * radiusMult * px.y,
    );
  ...
  function initialSplats(): void {
    const cr = FLUID_CFG.cursorSize * 3.0;
    const seeds: Array<{ x: number; y: number; c: RGB }> = [
      { x: W * 0.20, y: H * 0.75, c: FLUID_CFG.c1 },
      { x: W * 0.78, y: H * 0.25, c: FLUID_CFG.c3 },
      { x: W * 0.50, y: H * 0.50, c: FLUID_CFG.c2 },
      { x: W * 0.35, y: H * 0.35, c: FLUID_CFG.c1 },
      { x: W * 0.65, y: H * 0.68, c: FLUID_CFG.c3 },
    ];
    seeds.forEach(s => {
      mesh.material = mSplat;
      mSplat.uniforms.source.value = dFBO0.texture;
      ...
      mSplat.uniforms.color.value.set(s.c[0] * 0.20, s.c[1] * 0.20, s.c[2] * 0.20);
      renderer.setRenderTarget(dFBO1);
      renderer.render(scene, camera);
      swapD();
    });
  ...
  const spotlight = document.getElementById('spotlight') as HTMLDivElement | null;
  ...
  function cycleColor(dt: number): RGB {
    colorPhase = (colorPhase + dt * 0.18) % 1;
    const t = colorPhase;
    if (t < 0.33) return lerpC(FLUID_CFG.c1, FLUID_CFG.c2, t / 0.33);
    if (t < 0.66) return lerpC(FLUID_CFG.c2, FLUID_CFG.c3, (t - 0.33) / 0.33);
    return lerpC(FLUID_CFG.c3, FLUID_CFG.c1, (t - 0.66) / 0.34);
  }
  ...
  const GHOSTS: GhostConfig[] = [
    { ax: 0.38, ay: 0.30, fx: 0.10, fy: 0.14, ph: 0,                cr: FLUID_CFG.c1, ca: FLUID_CFG.c3, ci: 0.09 },
    { ax: 0.30, ay: 0.36, fx: 0.08, fy: 0.11, ph: Math.PI,          cr: FLUID_CFG.c2, ca: FLUID_CFG.c1, ci: 0.07 },
    { ax: 0.28, ay: 0.28, fx: 0.06, fy: 0.09, ph: Math.PI * 0.5,    cr: FLUID_CFG.c3, ca: FLUID_CFG.c2, ci: 0.08 },
    { ax: 0.22, ay: 0.32, fx: 0.05, fy: 0.07, ph: Math.PI * 1.5,    cr: FLUID_CFG.c1, ca: FLUID_CFG.c2, ci: 0.06 },
  ];

  const BIRTH_POSITIONS: Array<[number, number]> = [
    [0.18, 0.78], [0.82, 0.22], [0.50, 0.52], [0.68, 0.72],
    [0.30, 0.30], [0.75, 0.58], [0.42, 0.18], [0.62, 0.42],
  ];
  ...
  (function tick(): void {
    ...
    if (Math.abs(dx) > 0.1 || Math.abs(dy) > 0.1) {
      ...
      splatDye(mX * px.x, mY * px.y, cycleColor(elapsed * 3), 0.12);
      pmX = mX; pmY = mY;
    } else {
      pmX = mX; pmY = mY;
    }
    ...
    GHOSTS.forEach(g => {
      const gx  = (0.5 + Math.cos(t * g.fx + g.ph) * g.ax) * W;
      const gy  = (0.5 + Math.sin(t * g.fy + g.ph) * g.ay) * H;
      ...
      splatDye(gx * px.x, gy * px.y, lerpC(g.cr, g.ca, (Math.sin(t * g.ci) + 1) * 0.5), 0.07);
    });
    ...
    if (spotlight) {
      ...
      const inner   = isLight ? 'rgba(60,110,200,0.065)' : 'rgba(29,78,216,0.15)';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const pageSize  = (): number => isVertical ? window.innerHeight : window.innerWidth;
  const maxScroll = (): number => (SECTIONS - 1) * pageSize();
  const lerp      = (a: number, b: number, t: number): number => a + (b - a) * t;
  const clamp     = (v: number, lo: number, hi: number): number => Math.max(lo, Math.min(hi, v));

  function setTransform(val: number): void {
    scroller.style.transform = isVertical ? `translateY(${-val}px)` : `translateX(${-val}px)`;
  }

  function updateProgress(val: number): void {
    const max = maxScroll();
    progressLine.style.transform = `scaleX(${max > 0 ? val / max : 0})`;
  }

  function updateBookmarks(idx: number): void {
  ...
  function startAnimate(): void { if (!rafId) rafId = requestAnimationFrame(animate); }
  ...
  bookmarksEl.forEach(bm => {
    bm.addEventListener('click', () => {
      const idx = parseInt(bm.dataset['idx'] ?? '0', 10);
      goTo(idx);
    });
  });
  ...
  summaryLinks.forEach(link => {
    link.addEventListener('click', (e: Event) => {
      ...
      goTo(parseInt(link.dataset['target'] ?? '0', 10));
    });
  });
  ...
  if (layoutToggle) {
    layoutToggle.addEventListener('click', () => {
      ...
      (layoutToggle.querySelector('.layout-icon-v') as HTMLElement).style.display = isVertical ? 'none'  : 'block';
      (layoutToggle.querySelector('.layout-icon-h') as HTMLElement).style.display = isVertical ? 'block' : 'none';
